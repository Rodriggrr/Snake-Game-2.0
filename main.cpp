...
{
    if ((snake[0].coord.X == fruit.coord.X && snake[0].coord.Y == fruit.coord.Y) || (snake[1].coord.X == fruit.coord.X && snake[1].coord.Y == fruit.coord.Y))
    {
        points++;
        size++;
        return true;
...
int snake()
{
    ...
    while (true)
    {
        ...
        if (!food) //----------------------------- procedimentos para fazer caso a comida tenha sido comida; 
        {          //----------------------------- basicamente colocar mais um ponto, criar outra comida e mudar a pontuação.
            change_title();
            fix_snake(snake);
            put_food(snake, fruit);
            food = true;
        ...
        delete_last(lastDot);                   // deleta o ultimo ponto, para não ser uma cobra infinita.
        choice = verify_side(snake, choice);    // verifica se o lado não é o contrário.
        if (choice == 'f'){
            put_food(snake, fruit);
            choice = 'd';
        }
        snake = mov_snake(snake, choice);       // move a cabeça da cobra para o lado desejado.
        if (verify_col(snake) || choice == 'm') // condição para perder.
            break;
        lastDot = snake[snake.size() - 1];      // variavel recebe o ultimo ponto da cobra antes de mudar, para ser apagado.
        snake = correct_pos(snake);             // corrige a posição na tela, para reaparecer do outro lado, caso necessario.
...
int menu()
{
    ...
    char choice = 'a', aux;
    while (choice != ' ')
    { // apenas sai quando o jogador apertar espaço.
        aux = choice;
    ...
    if (aux == 'd') // dependendo da escolha, uma ação é realizada, no caso jogar ou sair.
        return 0;
    else if(aux == 's'){
        help();
        clrscrn();
